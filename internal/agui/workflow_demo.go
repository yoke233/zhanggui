...
func (h *Handler) runDemoNew(ctx context.Context, gw *gateway.Gateway, emitter *Emitter, st *RunState, req RunRequest, session *Session) error {
	...
	if err := emitter.Emit(Event{"type": "TEXT_MESSAGE_CONTENT", "messageId": messageID, "delta": "请在 UI 表单中填写 demo 参数，然后提交。\n"}); err != nil {
		return err
	}
	...
	toolArgs := map[string]any{
		"title":       "demo：请输入任务参数",
		"description": "这是一个最小 tool 闭环示例（后续会替换为真实任务输入）。",
		"schema": map[string]any{
			"type": "object",
			"properties": map[string]any{
				"topic": map[string]any{"type": "string", "title": "topic"},
			},
	...
	if err := emitter.Emit(Event{"type": "TOOL_CALL_START", "toolCallId": toolCallID, "toolCallName": toolName, "parentMessageId": messageID}); err != nil {
		return err
	}
	if err := emitter.Emit(Event{"type": "TOOL_CALL_ARGS", "toolCallId": toolCallID, "delta": string(argsBytes)}); err != nil {
		return err
	}
	if err := emitter.Emit(Event{"type": "TOOL_CALL_END", "toolCallId": toolCallID}); err != nil {
		return err
	}
	...
	if err := emitter.Emit(Event{"type": "TEXT_MESSAGE_CONTENT", "messageId": msg2, "delta": "已收到表单输入，进入 interrupt 演示：将请求审批后继续。\n"}); err != nil {
		return err
	}

...
1. `draft`：创建方案，添加 WorkItem，编辑依赖关系
...
`approved → executing` 转换时：
1. 遍历 Initiative 下所有 WorkItem
2. 无依赖的 → 直接 queue
3. 有依赖的 → 标记为 accepted，等待依赖完成
...
POST   /api/initiatives/{id}/approve                 # proposed → approved
POST   /api/initiatives/{id}/reject                  # proposed → draft（附 feedback）
POST   /api/initiatives/{id}/execute                 # approved → executing
...
- `internal/core/initiative.go`：Initiative、InitiativeItem、ThreadInitiativeLink
- `internal/core/initiative.go`：InitiativeStore 接口

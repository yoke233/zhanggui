...
- 唯一位置：`<outbox_repo>/workflow.toml`
...
- 对于 SQLite backend：assign 只是更新 issue 的 `assignee` 字段（`issues.assignee`），不存在平台权限问题，但也不具备平台级强认证（见 `docs/operating-model/outbox-backends.md`）
...
PR 闭环图（概览，包含并行 Review + QA + CI，以及 integrator 的收敛点）：

```mermaid
flowchart TD
  Issue[(Outbox Issue<br/>IssueRef)]

  PR[Worker: 提交或更新 PR] --> CI[CI: checks]
  PR --> R[Role Lead: code review]
  PR --> Q[QA: 回归/验收]

  CI -->|pass| G{Gate: all pass<br/>for same head_sha}
  R -->|approved| G
  Q -->|pass| G

  CI -->|fail| F[Integrator-Lead: 汇总 FixList vN]
  R -->|changes requested| F
  Q -->|fail| F
  G -->|no| F

  F -->|Next: worker 修复<br/>(new RunId)| PR

  G -->|yes| M[Integrator: merge<br/>(or role lead merge if no permissions)]
  M --> V[Integrator: 集成验收]
  V --> D[Writeback: state:done + evidence]
  D --> C[Approver: close issue (if required)]

  CI -->|evidence| Issue
  R -->|verdict + evidence| Issue
  Q -->|verdict + evidence| Issue
  F -->|single FixList + routing| Issue
  M -->|merge info| Issue
  V -->|integration evidence| Issue
  D -->|final status| Issue
```

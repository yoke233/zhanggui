...
> **设计决策**：需求管理和审核已上提到 Secretary Layer（Issue 级），Pipeline 只负责纯执行。
> "做什么"由 Issue 决定，Pipeline 通过 `issue_id` 关联 Issue 后，读取 plan 文件快照内容进入 requirements → implement 流程。
> 详见 [spec-secretary-layer.md](spec-secretary-layer.md)。

| Stage ID | 职责 | 执行者 | 典型耗时 |
|---|---|---|---|
| `requirements` | 结构化需求描述 | Agent（ACP） | 1-3 min |
| `worktree_setup` | 创建 git worktree + 复制文件 | Git | < 10s |
| `implement` | 根据需求描述实现代码 | Agent（ACP） | 5-30 min |
| `code_review` | 审查实现质量 | Agent（ACP） | 3-10 min |
| `fixup` | 修复 review 问题 | Agent（ACP） | 3-15 min |
| `e2e_test` | 端到端测试（full 模板包含） | Agent（ACP） | 2-10 min |
...
stage:
  name: implement
  role: "worker"                 # 执行该阶段的角色（由 role_bindings.pipeline.stage_roles 提供默认值）
...
### Executor/Stage 通信层（ACPClient 生命周期）

Pipeline 的调度、状态机、Reactions Engine 保持不变；通信抽象统一为 ACPClient + SessionManager。
...
- `AcquireSession`：按策略复用已有 session，或 `LoadSession/NewSession` 创建。
- `ReleaseSession`：按策略决定“保活待复用”或“立即回收”。

会话策略（`pipeline.acp.session_strategy`）：

| 策略 | 行为 | 适用场景 |
|---|---|---|
| `per_stage` | 每个 Stage 独立会话（结束即回收） | 最保守，易审计 |
| `hybrid`（默认） | `worker`（implement/fixup）复用；`reviewer` 由对应 role 的 session 策略决定（默认复用） | 性能与隔离平衡 |
| `per_pipeline` | 一个 Pipeline 内所有 Agent Stage 尽量复用同一会话 | 极致减少上下文重建 |

执行细节：
1. Executor 进入 Stage 后，根据 `stage.role`（或 `role_bindings.pipeline.stage_roles`）解析 `RoleProfile` 并调用 `AcquireSession(...)`。
2. 若命中可复用 session，直接 `Prompt`；否则按顺序尝试：`LoadSession -> NewSession`。
3. 流式处理 `session/update` 事件，累计输出、工具结果和阶段日志。
4. 收到 `stopReason = end_turn` 后，进入结果解析与 Artifact 写入。
5. 调用 `ReleaseSession(...)`：按策略保活或回收。
6. Pipeline 完成/终止时统一 `acpClient.Close(ctx)` 回收该 Pipeline 下所有 session。

约束：
- Prompt 注入、重试预算、`on_failure` 到 Reactions 的编译规则不变。
- 开启 `reviewer` 复用时，review prompt 必须注入 reset 语义（“仅基于当前代码快照重新审查”）以降低审查漂移。
- 当 `Prompt` 返回 session 不存在/传输断连错误时，必须自动回退 `NewSession` 并重放必要上下文。
- RoleResolver 必须保证 `role.capabilities` 不超过对应 Agent 的 `capabilities_max`。
...
> 旧版的”规格生成/规格审核”阶段已移至 Secretary Layer 的 Issue 级别处理。
...
- Reviewer 角色配置 `session.reuse=true` 时，review → fix → review 循环可复用 reviewer session（否则每轮新建）
...
2. **Issue.Template** — Issue 创建时指定或 AI 审核建议的模板（P2a）
3. **项目配置映射** — 项目配置中定义 `label_mapping: { "bug": "quick" }`（仅适用于 GitHub Issue 触发的 Pipeline，P3）
4. **AI 推断** — 将需求描述发给 Agent（ACP），返回模板名称
...
2. 对每条 Pipeline，重建内存结构（重新创建 humanCh、cancelFn、SessionManager 缓存等）
3. 根据状态分别处理：
...
5. 若策略开启 `prefer_load_session`，恢复时优先 `LoadSession`，失败再 `NewSession` + 上下文补水
...
| 手动暂停 | 用户主动 Pause | 中断当前 Stage 的 ACP 会话（见下方 Pause/Resume 语义），暂停 |

### Pause/Resume 语义

- **Pause**：取消当前 Stage 执行 context，并优先调用 `acpClient.Cancel(ctx, req)` 终止当前 request；session 是否保留由 `pipeline.acp.pause_keep_session` 与 `session_strategy` 决定（保守模式会回收，复用模式可保活）
- **Resume**：优先复用 Pause 前缓存的 session（仍有效时）；不可用则按 `LoadSession -> NewSession` 重建，再从当前 Stage 头重新执行（先清理脏状态：`git checkout . && git clean -fd`，并注入恢复提示）
...
| `change_role(role)` | 切换阶段角色并重跑当前阶段 | 当前角色效果不稳定，切换到备用角色 |
...
- 超时时通过 `context.Cancel()` + `acpClient.Cancel(ctx, req)` 中断当前 request；是否回收 session 由会话策略决定（复用策略可保留用于重试）
...
- 重试时使用相同角色，除非人工通过 `change_role` 更换
...
type Notification struct {
    ...
    PipelineID string
    ProjectID  string
    IssueID    string  // 关联的 Issue ID（如有），便于端到端追踪
...
| **DAG Scheduler 自动创建** | **Issue 审核通过后，为每个 Issue 自动创建 Pipeline（1 Issue = 1 Pipeline）** | **P2a ✅** |
...
- 模板由 Issue.Template 字段指定
- requirements 阶段读取 Issue 关联的 plan 文件快照（issue_attachments），合并注入 agent prompt
- Agent 内部处理并行执行，系统不拆解子任务
...
| 事件类型 | 触发时机 | 数据 |
|---------|---------|------|
| `issue_created` | Issue 创建 | issue_id, project_id |
| `issue_reviewing` | AI 审核开始 | issue_id |
| `issue_review_done` | 单个 Issue 审核完成 | issue_id, verdict, score |
| `issue_dag_proposed` | 跨 Issue 依赖分析完成 | project_id, edges, conflicts |
| `issue_approved` | Issue 批准执行 | issue_id |
| `issue_queued` | Issue 等待依赖 | issue_id |
| `issue_ready` | Issue 可调度（入度归零） | issue_id |
| `issue_executing` | Issue Pipeline 启动 | issue_id, pipeline_id |
| `issue_done` | Issue 完成 | issue_id |
| `issue_failed` | Issue 失败 | issue_id, error |
| `issue_closed` | Issue 关闭 | issue_id, reason |
| `issue_changed` | Issue 字段变更 | issue_id, field, old_value, new_value |

这些事件和现有的 `stage_*`、`pipeline_*` 事件共存于同一个 Event Bus。

> 详细的 Secretary Layer 设计（Secretary Agent、Issue 模型、两阶段 AI 审核、DAG Scheduler、Workbench UI）见 [spec-secretary-layer.md](spec-secretary-layer.md)。
...
> 注意区分两层并行：
> - **Issue 级并行**（P2a）：多个 Issue 的 Pipeline 并行执行，由 DAG Scheduler 调度

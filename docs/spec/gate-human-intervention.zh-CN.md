...
逻辑：
- `retry`：重置 retry_count，step → `StepPending`，重新调度
- `skip`：标记 `StepDone`（带 `skipped: true`）
- `override_pass`：gate step 直接 pass

#### 查询待处理列表

```
GET /issues/{issueID}/pending-decisions   — 某 issue 下所有等待人类的 step
GET /pending-decisions                     — 全局待处理列表（Dashboard 用）
```

返回 `StepBlocked` + `StepWaitingGate` 的 step，附带最新的 StepSignal 作为上下文。
...
| 阶段 | 内容 | 工作量 |
|------|------|--------|
| **P0** | `StepSignal` 模型 + Store + SQLite 表 | 小 |
| **P0** | `mcp-serve` 子命令：exec 工具 (step_complete, step_need_help) + gate 工具 (gate_approve, gate_reject) + step_context | 中 |
| **P0** | `handleSuccess` 改造：检查 SignalNeedHelp / SignalComplete | 小 |
| **P0** | `finalizeGate` 改造：检查 SignalApprove / SignalReject | 小 |
| **P0** | `buildStepMCPFactory` 改造：按 step type 注入 env | 小 |
| **P1** | `POST /steps/{id}/decision` + `POST /steps/{id}/unblock` API | 小 |
| **P1** | `EventStepNeedHelp` 事件 + WebSocket 广播 | 小 |
| **P1** | 前端审批/阻塞面板 | 中 |
| **P2** | `hybrid` gate mode（AI + 升级阈值） | 中 |
| **P2** | Dashboard 待处理汇总面板 | 中 |
| **P2** | 信号时间线 UI | 小 |

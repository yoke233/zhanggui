...
1. **字符预算硬限** — 上游 Artifact 限 4000 字符、Issue 限 800 字符，大量有价值的信息被截断
...
║  │ Phase A: 材料预投放 (新增)                                    │   ║
║  │                                                              │   ║
║  │  StepContextBuilder.Build(step, exec)                        │   ║
║  │    │                                                         │   ║
║  │    ├── 读取 Issue (完整 body, 不截断)                         │   ║
║  │    ├── 读取所有上游 Artifact (完整 ResultMarkdown)             │   ║
...
以下材料在每次 Step 执行时自动生成（如果数据存在）：

| 文件 | 来源 | 内容 | 生成条件 |
|------|------|------|----------|
| `issue.md` | `Store.GetIssue()` | Issue 完整 Title + Body + Priority + Labels | Issue 存在 |
| `upstream/<name>.md` | `Store.GetLatestArtifactByStep()` | 前置 Step 的完整 `ResultMarkdown` | 有上游 Artifact |
| `acceptance.md` | `step.AcceptanceCriteria` | 编号列表的验收标准 | AcceptanceCriteria 非空 |
| `gate-feedback.md` | `step.Config["last_gate_feedback"]` + `step.Config["rework_history"]` | Gate 反馈 + 历史 | 存在反馈 |
...
| `related-issues.md` | 关联 Issue 列表 | 新增 |

## SKILL.md 动态生成

`SKILL.md` 由 `StepContextBuilder` 在运行时动态生成，内容根据实际写入的材料文件生成索引表：
...
| 文件 | 职责 |
|------|------|
| `internal/skills/context_builder.go` | `StepContextBuilder` — 根据 Step/Issue/Artifacts 生成 step-context 目录 |
...
| 文件 | 变更 |
|------|------|
| `internal/adapters/executor/acp.go` | `ACPExecutorConfig` 新增 `StepContextBuilder` 字段；executor 中调用 Build + 注入 extraSkills + defer Cleanup |
...
| `internal/application/flow/execution_input.go` | `BuildExecutionInputForStep` 新增 `hasStepContext` 参数，追加材料引用提示 |
| `internal/platform/bootstrap/bootstrap_engine.go` | 装配 `StepContextBuilder` 并注入 `ACPExecutorConfig` |

## StepContextBuilder 接口
...
func (b *StepContextBuilder) Build(ctx context.Context, parentDir string,
    step *core.Step, exec *core.Execution) (dir string, err error)
...
if cfg.StepContextBuilder != nil &&
    ...
    ctxParentDir := filepath.Join(cfg.DefaultWorkDir, ".ai-workflow", "step-contexts")
...
### BuildExecutionInputForStep 签名变更

```go
func BuildExecutionInputForStep(
    profile *core.AgentProfile,
    snapshot string,
    step *core.Step,
    hasPriorTurns bool,
    ...
    reworkTmpl string,
    continueTmpl string,
    hasStepContext bool,        // ← 新增
) string
```

### 新增 Reference Materials 章节

当 `hasStepContext == true` 时，在 Acceptance Criteria 之前插入：
...
| 层 | 内容 | 字符限制 | 用途 |
|----|------|----------|------|
| **Prompt 层** (BriefingSnapshot) | 摘要：Objective + Issue 摘要 + Upstream 摘要 + Manifest 精简 | ≤12000 字符 | Agent 的即时任务理解 |
| **文件层** (step-context) | 完整版：Issue 全文 + Artifact 全文 + Manifest 全量 JSON | 无限制 | Agent 按需深入 |

当前阶段 Briefing 预算不变；未来可以考虑在有 step-context 时**主动缩减** Briefing 预算（例如 Issue 摘要改为仅 Title），以节省 prompt token。

## 生命周期管理

```
StepContextBuilder.Build()
    → 写入 .ai-workflow/step-contexts/exec-<id>/step-context/
...
ACPStepExecutor defer
    ...
    → 删除 .ai-workflow/step-contexts/exec-<id>/ 整个目录
...
| 用例 | 验证点 |
|------|--------|
| `TestBuild_FullMaterials` | Issue + upstream + acceptance + manifest 均生成 |
| `TestBuild_NoIssue` | Issue 不存在时跳过 issue.md |
| `TestBuild_NoUpstream` | 无上游 Step 时不生成 upstream/ |
| `TestBuild_GateFeedback` | 有 gate 反馈时生成 gate-feedback.md |
| `TestBuild_SkillMD_Index` | SKILL.md 索引表包含所有实际文件 |
| `TestBuild_LargeArtifact` | 大体量 Artifact 不被截断 |
...
| 场景 | 验证点 |
|------|--------|
| E2E Step 执行 | step-context 目录在执行前存在，执行后被清理 |
| Agent 实际读取 | Agent 的输出引用了 step-context 中的文件内容 |
| 降级模式 | `StepContextBuilder = nil` 时行为与现有完全一致 |
| Session 复用 | 复用会话时 step-context 被刷新（新 execution 的材料） |

## 相关文件

| 文件 | 职责 |
|------|------|
| `internal/skills/context_builder.go` | **新增** — StepContextBuilder 核心实现 |
...
| `internal/platform/bootstrap/bootstrap_engine.go` | **修改** — 装配 StepContextBuilder |

...
| `spec_gen` | 生成 OpenSpec 全套文件 | Claude + OpenSpec | 3-8 min |
| `spec_review` | 审核 spec 一致性和可行性 | Claude | 2-5 min |
| `worktree_setup` | 创建 git worktree + 复制文件 | Git | < 10s |
| `implement` | 根据 spec/需求实现代码 | Codex 或 Claude | 5-30 min |
| `code_review` | 对照 spec 审查实现 | Claude | 3-10 min |
| `fixup` | 修复 review 问题 | Codex 或 Claude | 3-15 min |
| `e2e_test` | 端到端测试（可选） | Codex | 2-10 min |
| `cleanup` | openspec archive + worktree 移除 + 分支清理 | Git + OpenSpec | < 10s |
...
spec_gen     → artifacts["spec_path"], artifacts["tasks_md"]
...
```
full:      requirements → spec_gen → spec_review → worktree_setup
           → implement → code_review → fixup → merge → cleanup
...
- 执行 `openspec archive` 归档 spec 文件到 `openspec/changes/archive/`
...
```
full     → 新功能、涉及多模块、需要架构设计、API 变更
standard → 中等功能、单模块改动、不需要详细设计文档
...
custom_templates:
  ...
  with-e2e:
    stages: [requirements, spec_gen, spec_review, worktree_setup,
             implement, code_review, fixup, e2e_test, merge]
    defaults:
...
| `reject(stage)` | 回退到指定阶段重新执行 | spec 不满意，需要重新生成 |
